...
I explained the big parts of computers, now I'll focus on ML.
...
## Questions when explaining ML
1. How duality of data structures and algorithms translates to implicit coding of ml? Is it that data structures are intact, just algorithms change? Unobvious given data heaviness of ML.
2. When I say that data is the world that model operates in, is that correct? Is it better to perceive it through simple input/output prism?
3. How do you square the difference between explicit and implicit coding in regards to the enormous amount of data we use for the latter? => We need to model the phenomena, in explicit coding that is in the head of the coder (he knows what is the phenomena, and what output he wants), in implicit coding machine has to do that from scratch, so besides learning transformations it also has ingrained something about the surrounding phenomena, something that is already in the head of the coder.
...
Alright, so you understand that humans are tool-makers/ users, that computers are tools to manipulate information, that this is extremely powerful because information might be more fundamental than matter, and after manipulating it computer can use physical leverage to transform abstract changes into the real world, and if it is turing complete the set of all these possible changes is huge, uncomparable to hammer.

You then even understand the parts of computer, how memory is made, that software is all about data structures and algorithms, even what are the main data structure types and algorithms types. Once you're ready to code to solve some problem you even understand the differences in coding language, the choices each one makes based on the problem they're solving, the fact that programs are just divide and conquer of smaller programs, you get what libraries are, you're connected to internet and bla bla bla -- you're fucking ready to solve some problems.

What now?

Now we can talk about explicit vs implicit coding. While in principle those coding paradigms solve the same set of problems (all computable functions), in principle they are very different, so it is useful to differentiate the two.

Explicit coding is what we've been doing for the past +-60-70 years. We give very specific explicit instructions step-by-step of what computer needs to do with the information. 

Implicit coding solves the type of problems it is hard to solve with explicit coding. Think about using it to tell whether image has a bee. In principle you can do it with some enormous amount of if/else loops. In practice humans don't know how to do it.

Here comes implicit coding. Since we can't specify exactly what are the steps that computer should be following, we instead create an environment where it itself learns/ finds those steps.

This is called machine learning. And the main loop that it follows is: guess => feedback => update.

We make computer make a guess of what could be a good transformation to make, we then grade it and give feedback, and then make it update its guess to a new one. We run this loop until it performs well on the task.

Now let's go into more detail about it.

1. First step is to identify a mathematical architecture that model will be using. Assumptions and limitations. You want hypothesis space to be big enough to have viable solution in it, but not too big that search takes forever.
2. Second step is to generate random weights for it and make a guess using them and some input data we give.
3. Third, we evaluate that guess. We compare it to the desired output. We define the feedback function.
4. Forth we define the updating function which tells our model how much it should change the weights (the transformation it will do to the data), and we evaluate that transformation.
5. We repeat that until the guesses are acceptable.

A lot of questions here, not sure I'm describing it well also❓.

The main question I had when I was describing this is how it bridges/ connects to explicit coding. 

If both of these are just types of coding, they should be similar, they should follow familiar steps. Yet, when describing implicit coding there's one thing that is absolutely alien to explicit coding.

It is the enormous amount of data we have to use in machine learning, which we never use for explicit coding.

Explicit coding is: inputs => transformations => output
In implicit coding we're going a step beyond and make computer find transformations as we can't state them exactly => learning algorithm with feedback, updating etc.(inputs => transformations => output)

And we hope that that learning algorithm would find the right transformations to make our inputs into nice lovely outputs.

So far so good.

**But what's up with all the data we collect for implicit coding, why we don't have that nasty part in explicit coding?**

After several independent conversations I do think I have arrived at an amazing answer which reveals an important insight.

We collect all that data to model the phenomena, once we have modeled it, we can talk transformations. 

In explicit coding that phenomena modeling is usually already done, it is in the head of the coder, now he just needs to find right transformations. 

But in implicit coding model starts from scratch, it has none of this knowledge about this phenomena, so besides figuiring out transformations, it needs to incorporate the knowledge about the phenomena. 

A perfect example is medical diagnosis, say tumor identification based on scans. If I were to write an explicit code (who has no radiologist practice) to identified tumors I would first need to learn about them. I would need to learn what to look for and so on. I would need to model the phenomena, both the inputs and the outputs (to then specify the right transformations). But for example if you have radiologist that can code, he already has that phenomena knowledge in his head after decades of practice. He already has collected all this data over the years, the question for his is whether he can translate this internal knowledge into specific rules for explicit coding (spoiler he can't really do it, but that's just the detail of the example, not necessity). Now consider ml model, besides guessing transformations, it needs to generate some knowledge about the phenomena, it needs to model tumor detection phenomena that is representable from real world. It might not need to know what day of the week is, but it needs to see the scan and whatever else needed for tumor identification.

So this huge data collection part, is yes, of course about iteratively finding the right transformations, but it is also about modeling the phenomena, something that is usually already in the heads of coders doing explicit coding.

**So large sworths of data serve two purposes: creating a good representation of the phenomena that is similar in important ways to reality, and then using that representation to iteratively find desired transformations to reach right outputs.**
